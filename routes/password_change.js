...
router.post('/', function (req, res, next) {
   ...
   * fields: 테이블의 필드 정보
   */
    mysqlDB.query('select * from account where id=? and pw=SHA(?)', [userId, userPw], function (err, rows, fields) {
        if (!err) {                         //오류가 없을 때 아래의 코드 실행
            if (rows[0] != undefined) {     //row가 존재하면 비밀번호 변경
                mysqlDB.query('update account set pw=SHA(?) where id=?', [userPwNew, userId], function (err, rows, fields) {
                    if (!err) {             //오류가 없을 때 아래의 코드 실행
